...
[3.3.9]
- fixed a launch issue with 1.21.1 && 1.20.1 introduced in v3.3.8

[3.3.8]
- fixed `is_on_shoulder` not working on 1.21.9+
- added attachment points for `parrot_left` & `parrot_right` to allow manually controlling the position of shoulder parrots on the player model (supports animation)
- added an option to have parrots adjust their position automatically to match custom player model animations (ignores rotation and uses the vanilla offsets, but will move to match the player shoulder position from animations, is disabled if the parrot offsets are used, or via the settings)
- parrots on the left shoulder will now return `0` for the animation variable `id`, and `1` for the right shoulder
- fixed `head_yaw` and `head_pitch` for shoulder parrots (also need ETF update to work)

[3.3.7]
- fixed a texture override issue when the model doesn't declare all parts or uses attach=true and every part declared the same texture override
- fixed several issues where third party mixins cancelling certain renders would mess up the animation values of following renders (also need to update ETF for the full fix)
- fixed `is_in_ground` not working for arrows and tridents on the floor (previously only worked on walls or ceilings)
- fixed `is_in_hand`, `is_in_gui`, `is_in_item_frame`, `is_on_head`, `is_in_ground` (for arrow stuck in entity), breaking on 1.21.9+
- fixed an issue with the Spanish Argentina localization file

[3.3.6]
- fixed an issue where ASM compiled maths had an upper limit on animation size (`MethodTooLargeException`)
- fixed an issue where the first model variated of each frame could break
- fixed villager model exports
- updated to 26.3
...
[3.3.2]
- added API methods to manually trigger animations for specific Entities or EntityStates
- bumped api version to 11
- fixed a crash in MC 1.21.9 to 26.1

[3.3.1]
- fixed a crash on MC 1.21.1
...
- added new item/block attachment points for models:
  - `head_item` (Almost all humanoid models with a head armor slot, including player)
  - `sulfur_cube_block` (sulfur_cube internal block) (This one is special as it will be active by default, toggleable in settings)
  - `enderman_block`
  - `villager_item`
  - `witch_item`
  - `panda_item`
...
- added animation hooks to the `EMFAnimationAPI` to allow other mods to act before and after EMF animation, and even cancel it
- bumped `EMFAnimationAPI` version to 10
- fixed Essential mod emote's pausing player animations
- fixed Essential mod cosmetics not animating properly with first person hand animations

[3.2.4]
- added stinger part to baby bee models
- fixed the height_above_ground animation variable with block entities considering its own block as the ground
- fixed a startup crash with supplementaries and EMF v3.2.3 specifically
- added some more detailed failure messages for asm animation math errors

[3.2.3]
- fixed asm animation math failing when booleans were compared with the `==` or `!=` operators
- further fixes for entity layer renderers reusing the base model and its interaction with texture overrides
- fixed baby model part names mappings in 26.1 for zoglin, llama, trader llama, and all horses/donkeys
- fixed a startup crash

[3.2.2]
- fixed random models no longer working in v3.2

[3.2]
- significant performance improvement for custom animations (more than doubling frames when many animated entities are present)
- entity animation variables now reset when resource-packs are reloaded (e.g. var & varb)
- fixed models with texture overrides breaking for layer models (e.g. villager layers)
- reduced some log spam
- fixed new 26.1+ rabbit model part names
- fixed several baby model part name mappings in 26.1+
- further fixes for first person arm positioning when animated
- various minor fixes, mostly relating to layer models and 1.21.9+ consistency
- breaking changes made to the `EMFAnimationAPI`, mods that use this API may need to update, like ESF
- temporarily incompatible with ESF again, will be fixed soon

[3.1.1]
- Optimized resource loading time
...
- fixed model exporting on 26.1+
- fixed the config sometimes not saving in 26.1+
...
[3.0.17]
- fixed the position of the first person hand when using custom player models on 1.21.9+
- fixed the config not changing properly until a full game reload on 26.1
- fixed a crash that could occur when opening the config screen

[3.0.16]
- fixed exporting models in 26.1
- fixed many part names for 26.1 baby models (same may have been missed, please report them if so)
- EMF no longer allows baby models to fall back to their adult model in 26.1+ when no baby model is present
- fixed "value is not a boolean" animation error when a part is missing and its visibility is tested


[3.0.15]
- fixed baby pig models breaking in 26.1 even with no custom models loaded
- fixed rabbit model part name mappings in 26.1
- fixed rabbit model exports in 26.1
- this release also adds all the fixes from v3.0.13 to mc versions other than 26.1

[3.0.14]
- fixed ArrayIndexOutOfBoundsException crash

[3.0.13]
- 26.1 support for fabric and forge (note baby models changed significantly)
- fixed an issue with the animation variable api
- bumped the animation api version
- fixed shoulder parrots not animating in 1.21.9+
- fixed boat water patch models not animating in 1.21.9+
- fixed horse saddle OptiFine part names on 1.21.5+
- fixed the ifb() function causing a stack overflow
