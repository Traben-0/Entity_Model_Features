...
public class EMFCustomCodEntityModel<T extends LivingEntity> extends CodEntityModel<T> implements EMFCustomEntityModel<T> {
    ...
    private static final HashMap<String,String> optifineMap = new HashMap<>(){{
        put("tail_fin","tail");
        //only needs tail assigned for vanilla fish
    }};
    public EMFCustomCodEntityModel(EMFGenericCustomEntityModel<T> model) {
        ...
        super( EMFCustomEntityModel.getFinalModelRootData(
                CodEntityModel.getTexturedModelData().createModel(),
                model, optifineMap));
