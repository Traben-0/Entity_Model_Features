...
public class EMFModelPartRoot extends EMFModelPartVanilla {
    ...
    private Optional<Resource> getResource(Identifier id) {
        try {
            return Optional.of(MinecraftClient.getInstance().getResourceManager().getResource(id));
        }catch (Exception e) {
    ...
    public void discoverAndInitVariants() {
        ...
        if (getResource(propertyID).isPresent()) {
