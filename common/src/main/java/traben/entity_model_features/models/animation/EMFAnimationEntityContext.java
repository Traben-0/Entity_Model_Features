...
public class EMFAnimationEntityContext {
    ...
    private static int getLODFactorOfEntity() {
        ...
        if(EMF.config().getConfig().retainDetailOnLowFps && MinecraftClient.currentFps < 59){ // count often drops to 59 while capped at 60 :/
            float fpsPercentageOf60 = MinecraftClient.currentFps / 60f;
    ...
    public static float getDimension() {
            ...
            if (emfEntity == null || emfEntity.etf$getWorld() == null) {
                ...
                Identifier id = emfEntity.etf$getWorld().getDimension().getEffects();
                if (id.equals(DimensionType.THE_NETHER_ID))
                    return -1;
                if (id.equals(DimensionType.THE_END_ID))
                    return 1;
                else
                    return 0;
            }
...
//                (emfEntity instanceof CamelEntity camel && camel.isSitting())
    ...
    private static void doLimbValues() {
        ...
        if (emfEntity instanceof LivingEntity alive && !emfEntity.emf$hasVehicle()) {
